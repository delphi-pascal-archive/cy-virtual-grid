...
{   Component(s):
    ...
    Description:
    Component that generate cell coordonates to make a virtual grid.
...
type
  ...
  private
    ...
    FValidCells: Boolean;  // Allow to know if the some properties were changed since last generation
    FCellWidth: Word;
    FCellHeight: Word;
...
begin
  ...
  for y := fromCoordY to toCoordY do     // Criar linhas
  begin
    ...
    for x := fromCoordX to toCoordX do   // Criar colunas
    begin
...
procedure Register;
begin
  RegisterComponents('Cindy', [TcyVirtualGrid]);
end;
